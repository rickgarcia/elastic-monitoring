...
do
    ...
    scriptname="${filename%.*}"
    echo $scriptname
    es_response=$(curl -v -H "Content-Type: application/json" -s -X POST $protocol$endpoint:9200/_scripts/$scriptname -u $username:$password -d @$script)
...
curl -H "Content-Type: application/json" -s -o /dev/null -X DELETE $endpoint:9200/_xpack/watcher/watch/$1 -u $username:$password
es_response=$(curl -H "Content-Type: application/json" --w "%{http_code}" -s -o /dev/null -X PUT $protocol$endpoint:9200/_xpack/watcher/watch/$1 -u $username:$password -d @$1/watch.json)
